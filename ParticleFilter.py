...
def cramer_pf_PF(x_part,uN,P_x,N,y_K,S,k,T,Q,data):
    ...
    x_k = np.array(xk_resample).sum(axis=0).tolist()
    x_k = [x / N for x in x_k]
    x_Kk= np.random.multivariate_normal(f(x_k,uN,Q/N,Q)[0],f(x_k,uN,Q/N,Q)[1], N) -check
    w_k= [1/N] * N
    ...
    wKk = cramer_forward_particles(xK_resample,xk_resample,Q,N) -check
...
(x_part,uN,P_x,w_k,N,y_K,S,k,T,Q)=(x_particle[2],optionPrice.loc[1,'PriceChange'],P_0,w_particle,N,optionPrice.loc[1,'OptionPR'],optionPrice.loc[1,'StockPR'],optionPrice.loc[1,'StrikePrice'],optionPrice.loc[1,'TimeToMaturity'],Q)
def cramer_pf(x_part,uN,P_x,N,y_K,S,k,T,Q):
...
def cramer_elements(J_t,state,state_cov,M,N,Q,R,y_K,S_front,k_front,T_front,uN,isParticle,particle_data):
    ...
    if(isParticle==0):
        (x_particles_front,x_particles_back) = cramer_pf(state,uN,state_cov,N,y_K,S_front,k_front,T_front,Q)
    else if(isParticle==1):
        (x_particles_front,x_particles_back) = cramer_pf_PF(state,uN,state_cov,N,y_K,S_front,k_front,T_front,Q,particle_data)
    for j in range (0,M):
        for i in range(0,N):
...
def best_estimator(J_inv,P_x,state,P_w,P_v,N,data):
    ...
    for i in range(0,len(J_inv)):
        ...
        temp2 = inv(np.array(P_x[i])).diagonal().tolist()
        phi= [a*b for a,b in zip(temp1,temp2)]
        phi_sum = phi.sum()
        if(phi_sum>max_sum):
            optimal_bayesian = i
            max_sum = phi_sum
    (state_unscented,cov_unscented)= UKF_matrix(state[i],P_x[i],P_w,P_v)
    if(i!=2):
        state_particle = np.random.multivariate_normal(state[i],P_x[i], N)
        weight_particle = [1/N] * N
    elif(i==2):
        state_particle = data[0]
        weight_particle = data[1]
    return (state[i],P_x[i],state_unscented,cov_unscented,x_particle,w_particle,i)
...
def dataset():
    ...
    for e in range(1,optionPrice.shape[0]-1):
        ...
        (xuns_temp,state_uns,Cov_uns,Pa_O,P_w,P_v)=Unscented_KF_update(optionPrice.loc[e,'OptionPR'],xuns_temp,Pa_O,L,optionPrice.loc[e,'TimeToMaturity'],optionPrice.loc[e,'PriceChange'],optionPrice.loc[e,'StockPR'],optionPrice.loc[e,'StrikePrice'],P_w,P_v)
        (state_particle,x_particle,Cov_part,w_particle)=Particle_Filter(x_particle,optionPrice.loc[e,'PriceChange'],Q,w_particle,N,optionPrice.loc[e,'OptionPR'],optionPrice.loc[e,'StockPR'],optionPrice.loc[e,'StrikePrice'],optionPrice.loc[e,'TimeToMaturity'],e)
        
        PCRLB_EKF = cramer_elements(J_EKF,x_temp,P_temp,M,N,Q,R,optionPrice.loc[e+1,'OptionPR'],optionPrice.loc[e+1,'StockPR'],optionPrice.loc[e+1,'StrikePrice'],optionPrice.loc[e+1,'OptionPR'],optionPrice.loc[e+1,'PriceChange'],0,[])
        PCRLB_UKF = cramer_elements(J_UKF,state_uns,Cov_uns,M,N,Q,R,optionPrice.loc[e+1,'OptionPR'],optionPrice.loc[e+1,'StockPR'],optionPrice.loc[e+1,'StrikePrice'],optionPrice.loc[e+1,'OptionPR'],optionPrice.loc[e+1,'PriceChange'],0,[])
        PCRLB_PF = cramer_elements(J_PF,state_particle,Cov_part,M,N,Q,R,optionPrice.loc[e+1,'OptionPR'],optionPrice.loc[e+1,'StockPR'],optionPrice.loc[e+1,'StrikePrice'],optionPrice.loc[e+1,'OptionPR'],optionPrice.loc[e+1,'PriceChange'],1,[x_particle,w_particle])
        
        (x_temp,P_temp,xuns_temp,Pa_O,x_particle,w_particle,index) = best_estimator([J_EKF_inv,J_UKF_inv, J_PF_inv],[P_temp,Cov_uns,Cov_part],[x_temp,state_uns,state_particle],P_w,P_v,N,[x_particle,w_particle])
        
        (J_EKF,J_UKF, J_PF)= (PCRLB_EKF[0],PCRLB_UKF[0],PCRLB_PF[0])
        (J_EKF_inv,J_UKF_inv, J_PF_inv)= (PCRLB_EKF[1],PCRLB_UKF[1],PCRLB_PF[1])
        
        final_state.append(x_temp)
        final_state_var.append(P_temp)
        option_price.append(BS(x_temp,optionPrice.loc[e,'StockPR'],optionPrice.loc[e,'StrikePrice'],optionPrice.loc[e,'TimeToMaturity']))
        filter_used.append(index)
